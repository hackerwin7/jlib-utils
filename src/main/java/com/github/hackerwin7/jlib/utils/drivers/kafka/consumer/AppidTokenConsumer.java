...
import com.github.hackerwin7.jlib.utils.drivers.kafka.data.KafkaInfo;
...
public class AppidTokenConsumer {
    ...
    private BlockingQueue<KafkaMsg> queue = new LinkedBlockingQueue<KafkaMsg>(QUEUE_SIZE);
    private KafkaInfo info = null;
    ...
    public static KafkaInfo getInfo(String appid, String token) throws Exception {
        ...
        if(res.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
            ...
            String zkinfo = jdata.getString(JDQ_AUTH_ZK_ROOT_KEY);
            String topic = jdata.getString(JDQ_AUTH_TOPIC_KEY);
            String brokers = jdata.getString(JDQ_AUTH_BROKERS_KEY);
            KafkaInfo info = KafkaInfo.createBuilder()
                    .zks(zkinfo)
                    .topic(topic)
                    .brokers(brokers)
                    .build();
                ...
                if(messages.size() > 0) {
                    ...
                    for(JDQSimpleMessage msg : messages) {
                        ...
                        if(msg.getOffset() > endPosPool.get(partition))
                            break;
                        KafkaMsg kmsg = KafkaMsg.createBuilder()
     ...
     * @param _queue
     */
    public void setQueue(BlockingQueue<KafkaMsg> _queue) {
        queue = _queue;
    }
